...
export function checkConfigStatus() {
    const isSupabaseDemo = isDemoConfig;
    const isClerkDemo = isClerkDemoConfig;

    if (isSupabaseDemo || isClerkDemo) {
        const banner = document.createElement("div");
        banner.style.cssText = "background: linear-gradient(90deg, #F59E0B 0%, #D97706 100%); color: white; text-align: center; padding: 8px 16px; font-size: 0.9rem; font-weight: 500; position: sticky; top: 0; z-index: 1050; display: flex; align-items: center; justify-content: center; gap: 10px;";
        banner.innerHTML = `
            <span>⚠️ Running in Demo/Mock Mode with offline browser storage.</span>
            <button id="btn-config-credentials" class="btn btn-sm btn-light font-weight-bold" style="font-size: 0.75rem; border-radius: 4px; padding: 2px 8px;">Configure Database & Auth</button>
        `;
        document.body.insertBefore(banner, document.body.firstChild);

        const modalContainer = document.createElement("div");
        modalContainer.innerHTML = `
            <div class="modal fade" id="credentialsConfigModal" tabindex="-1" aria-labelledby="credentialsConfigModalLabel" aria-hidden="true">
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content glass-panel" style="background: white;">
                        <div class="modal-header border-0 pb-0">
                            <h5 class="modal-title fw-bold" id="credentialsConfigModalLabel">Configure Database & Auth</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body">
                            <p class="text-muted small">Enter your Clerk and Supabase configuration credentials below to enable live cloud services.</p>
                            
                            <div class="mb-3">
                                <label class="form-label small fw-semibold">Clerk Publishable Key</label>
                                <input type="text" id="clerk-key-input" class="form-control form-control-custom" placeholder="pk_test_...">
                            </div>

                            <div class="mb-3">
                                <label class="form-label small fw-semibold">Supabase Project URL</label>
                                <input type="text" id="supabase-url-input" class="form-control form-control-custom" placeholder="https://your-project.supabase.co">
                            </div>

                            <div class="mb-3">
                                <label class="form-label small fw-semibold">Supabase Anon Public Key</label>
                                <input type="text" id="supabase-key-input" class="form-control form-control-custom" placeholder="eyJhbGci...">
                            </div>
                        </div>
                        <div class="modal-footer border-0 pt-0">
                            <button type="button" class="btn btn-light" data-bs-dismiss="modal">Cancel</button>
                            <button type="button" id="btn-save-credentials" class="btn btn-primary-custom py-2 px-3">Save & Refresh</button>
                        </div>
                    </div>
                </div>
            </div>
        ...
        document.getElementById("btn-config-credentials").addEventListener("click", () => {
            document.getElementById("clerk-key-input").value = localStorage.getItem("KIDDYAI_CLERK_PUBLISHABLE_KEY") || "";
            document.getElementById("supabase-url-input").value = supabaseConfig.supabaseUrl || "";
            document.getElementById("supabase-key-input").value = supabaseConfig.supabaseKey || "";
            const modal = new bootstrap.Modal(document.getElementById('credentialsConfigModal'));
            modal.show();
        });

        document.getElementById("btn-save-credentials").addEventListener("click", () => {
            const clerkKey = document.getElementById("clerk-key-input").value.trim();
            ...
            try {
                if (clerkKey) {
                    localStorage.setItem("KIDDYAI_CLERK_PUBLISHABLE_KEY", clerkKey);
                } else {
                    localStorage.removeItem("KIDDYAI_CLERK_PUBLISHABLE_KEY");
                }

                if (supabaseUrl && supabaseKey) {
                    const configObj = { supabaseUrl, supabaseKey };
                    localStorage.setItem("KIDDYAI_SUPABASE_CONFIG", JSON.stringify(configObj));
                } else {
                    localStorage.removeItem("KIDDYAI_SUPABASE_CONFIG");
                }

                showToast("Credentials updated successfully! Refreshing...", "success");
                setTimeout(() => window.location.reload(), 1500);
            } catch (err) {
                alert("Invalid configuration credentials. Error: " + err.message);
            }
        });
    }
}
